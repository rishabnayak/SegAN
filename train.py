...
parser.add_argument('--ngpu', type=int, default=1,
                    help='number of GPUs to use, for now it only supports one GPU')
...
parser.add_argument('--seed', type=int, default=666,
                    help='random seed to use. Default=1111')
...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 1):
        ...
        for d in range(3):
            output_masked[:, d, :, :] = (
                input_mask[:, d, :, :].unsqueeze(1) * output).squeeze(1)
        if cuda:
        ...
        for d in range(3):
            target_masked[:, d, :, :] = (
                input_mask[:, d, :, :].unsqueeze(1) * target).squeeze(1)
        if cuda:
    ...
    print("===> Epoch[{}]({}/{}): Batch Dice: {:.4f}".format(epoch,
                                                             i, len(dataloader), 1 - loss_dice.item()))
    print("===> Epoch[{}]({}/{}): G_Loss: {:.4f}".format(epoch,
                                                         i, len(dataloader), loss_G.item()))
    print("===> Epoch[{}]({}/{}): D_Loss: {:.4f}".format(epoch,
                                                         i, len(dataloader), loss_D.item()))
    ...
    if epoch % 10 == 0:
        ...
        for i, data in enumerate(dataloader_val, 1):
            ...
            for x in range(input.size()[0]):
                ...
                dice = np.sum(pred_np[x][gt[x] == 1])*2 / \
        ...
        if mIoU > max_iou:
            max_iou = mIoU
            torch.save(NetS.state_dict(), '%s/NetS_epoch_%d.pth' %
                       (opt.outpath, epoch))
        vutils.save_image(data[0],
    ...
    if epoch % 25 == 0:
        ...
        optimizerG = optim.Adam(NetS.parameters(), lr=lr,
                                betas=(opt.beta1, 0.999))
        optimizerD = optim.Adam(NetC.parameters(), lr=lr,
                                betas=(opt.beta1, 0.999))
